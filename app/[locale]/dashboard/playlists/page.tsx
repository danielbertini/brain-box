...
import PlaylistsAddDialog from "./components/add-dialog";
...
export default function PlaylistsPage({ params: { locale } }: Props) {
  const limit = 30;
  ...
  const { data, mutate, size, setSize, isValidating, isLoading } =
    useSWRInfinite(
      (index) =>
        `/api/playlists/list?page=${index}&limit=${limit}&search=${searchQuery}&workplace_id=${appDetails?.workplace_id}`,
      fetcher
    );
  ...
  const renderHeader = () => {
    return (
      <>
        <UiPageTitle
          title={tPageDashboard("Menu.Playlists")}
          className="mt-8 mb-2"
        />
        <div className="flex flex-row gap-2 w-full items-center justify-between mb-4">
          ...
          <div>
            <PlaylistsAddDialog mutate={mutate} />
          </div>
        </div>
      </>
    );
